...
const game = {
  ...
  stats: null,
  overAt: 0,
  // Main Boss: free in a plain browser (web demo); in the app it is unlocked
  // with a purchase handled by the native side (RevenueCat).
  bossUnlocked: !bridge.native,
  unlockPending: false,
  price: ''
};
...
async function boot() {
  ...
  try {
    ...
    bridge.send('ready', { protocol: PROTOCOL });
  } catch (err) {
    console.error(err);
...
function showGameOver() {
  ...
  audio.setIntensity(0);
  setMode('over');
  bridge.send('gameOver', { score, wave: s.wave, kills: s.kills });
}

// ---------------------------------------------------------------- Main Boss (unlock + summon)

let unlockTimer = 0;

function onBossButton() {
  audio.unlock();
  if (game.unlockPending) return;
  if (game.bossUnlocked) {
    summonMainBoss();
    return;
  }
  // In the app: the native side shows the RevenueCat paywall. Freeze a live fight first.
  if (game.mode === 'playing') pause();
  game.unlockPending = true;
  syncBossButtons();
  if (!bridge.send('unlockBoss')) {
    game.unlockPending = false;
    syncBossButtons();
    toast('The store is not available');
    return;
  }
  // Safety net: never leave the button stuck if the app does not answer.
...
function summonMainBoss() {
  if (game.mode === 'title' || game.mode === 'over') play();
  else if (game.mode === 'paused') resume();
  const R = game.engine.layout.summonResults;
  const result = game.engine.summonBoss();
  if (result === R.Queued) banner('The Warlord approaches', 'He opens the next wave');
  else if (result === R.AlreadyHere) toast('The Crimson Warlord is already in the arena');
  else if (result === R.NotPlaying) toast('Start a run to face the Main Boss');
  return result;
}

function wireBridge() {
  document.body.classList.toggle('native', bridge.native);
  bridge.on('entitlement', (m) => {
    game.bossUnlocked = m.unlocked === true;
    game.price = typeof m.price === 'string' ? m.price.slice(0, 24) : '';
    syncBossButtons();
  });
  bridge.on('bossUnlocked', () => {
    const firstTime = !game.bossUnlocked;
    clearTimeout(unlockTimer);
    game.bossUnlocked = true;
    game.unlockPending = false;
    syncBossButtons();
    if (firstTime) toast('Main Boss unlocked');
    summonMainBoss();
  });
  bridge.on('purchaseCancelled', () => {
    clearTimeout(unlockTimer);
    game.unlockPending = false;
    syncBossButtons();
    toast('Purchase cancelled');
  });
  bridge.on('purchaseFailed', (m) => {
    clearTimeout(unlockTimer);
    game.unlockPending = false;
    syncBossButtons();
    toast(typeof m.message === 'string' && m.message ? `Store: ${m.message.slice(0, 120)}` : 'Purchase failed');
  });
  bridge.on('pause', () => pause());
  syncBossButtons();
}

function syncBossButtons() {
  const { bossUnlocked: unlocked, unlockPending: busy } = game;
  for (const el of document.querySelectorAll('[data-boss-button]')) {
    el.setAttribute('aria-busy', String(busy));
    el.disabled = busy;
  }
  for (const el of document.querySelectorAll('[data-boss-label]')) {
    const compact = !!el.closest('.tool');
    el.textContent = busy ? (compact ? 'Store…' : 'Opening store…') : unlocked ? (compact ? 'Summon boss' : 'Summon Main Boss') : compact ? 'Unlock boss' : 'Unlock Main Boss';
  }
  for (const el of document.querySelectorAll('[data-boss-price]')) el.textContent = unlocked ? '' : game.price;
  for (const el of document.querySelectorAll('[data-boss-sub]')) {
    el.textContent = !bridge.native ? 'Web demo: free · the Crimson Warlord' : unlocked ? 'Unlocked · summon him any time' : 'Face the Crimson Warlord';
  }
}
  ...
  for (const ev of s.events) {
    switch (ev.name) {
      ...
      case 'PlayerHurt':
        ...
        break;
      case 'EnemyDeath': {
        const warlord = ev.aux === L.enemyKinds.Warlord;
        const boss = ev.aux === L.enemyKinds.Boss || warlord;
        ...
        if (boss) {
          r.flashWhite = 1;
          r.shake(1);
          banner(warlord ? 'Crimson Warlord slain' : 'Dread Knight slain', warlord ? 'The Main Boss burns away' : 'The arena trembles');
          if (live) haptic('heavy');
          if (warlord) {
            fx.burst(ev.x, ev.y, 50, () => ({ type: 'ember', x: ev.x + (Math.random() - 0.5) * 90, y: Math.random() * 160, vx: (Math.random() - 0.5) * 120, vy: 60 + Math.random() * 160, drag: 0.8, life: 1.4 + Math.random(), size: 2 + Math.random() * 2, color: Math.random() < 0.5 ? '#ff7a1a' : '#ffcf7a' }));
          }
        } else r.shake(0.15);
        sfx?.death(boss);
        break;
      ...
      case 'Pickup':
        ...
        break;
      case 'BossSpawn': {
        const warlord = ev.value === L.enemyKinds.Warlord;
        r.shake(warlord ? 1 : 0.8);
        if (ev.aux === -1) banner('Enraged!', warlord ? 'The Warlord calls his knights' : 'The Dread Knight calls for aid');
        else {
          r.portal[ev.x < 1200 ? 0 : 1] = 1.5;
          if (warlord) {
            banner('The Crimson Warlord', 'The Main Boss enters the arena');
            if (live) audio.setIntensity(2);
            fx.burst(ev.x, 60, 30, () => ({ type: 'ember', x: ev.x + (Math.random() - 0.5) * 80, y: Math.random() * 150, vx: (Math.random() - 0.5) * 80, vy: 80 + Math.random() * 120, life: 1.2, size: 2.5, color: '#ff7a1a' }));
          }
        }
        if (live) haptic('heavy');
        sfx?.roar();
      ...
      case 'Slam': {
        const near = Math.abs(ev.x - p.x) < 600;
        r.shake(near ? 0.55 : 0.2);
        dust(fx, ev.x, 16);
        fx.add({ type: 'ring', x: ev.x, y: 4, size: 10, grow: 140, flat: 0.25, life: 0.45, color: '#ff7a1a' });
        sfx?.slam(near);
        if (live && near) haptic('heavy');
        break;
      }
      case 'PerfectDodge':
...
function updateHud(s, dt) {
  ...
  if (s.bossHp >= 0) {
    set('boss-fill', (s.bossHp * 100).toFixed(1), width);
    set('boss-name', s.bossKind, (el, v) => (el.textContent = v === L.enemyKinds.Warlord ? 'The Crimson Warlord' : 'The Dread Knight'));
  }
  // Nothing to summon while he is here (or already on his way).
  set('btn-boss-hud', s.bossKind === L.enemyKinds.Warlord || s.warlordQueued === 1, (el, v) => (el.hidden = v));
}
...
function wireUi() {
  ...
  for (const el of document.querySelectorAll('[data-boss-button]')) el.addEventListener('click', onBossButton);

  input.onCommand = (cmd) => {
    if (cmd === 'pause') game.mode === 'paused' ? resume() : pause();
    else if (cmd === 'mute') toggleMute();
    else if (cmd === 'vision') toggleVision();
    else if (cmd === 'boss') onBossButton();
    else if (cmd === 'confirm') {
      if (game.mode === 'title' || game.mode === 'over') play();
      else if (game.mode === 'paused') resume();
