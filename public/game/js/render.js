...
const DRAW_SCALE = { heroine: 1.4, knight: 0.84, rogue: 0.8, trap: 0.9, fx: 1.35 };
const KIND_COLOR = ['#ff5a5f', '#7fe8ff', '#ff2a3a'];
...
export class Renderer {
  ...
  draw(snap, dt, { paused = false } = {}) {
    ...
    const p = snap.player;
    for (const e of snap.enemies) this.#shadow(ctx, e.x, e.y, 30 * e.scale, e.state === this.FS.Dead ? 1 - e.deathTimer / 1.6 : 1);
  ...
  #drawEnemy(ctx, e) {
    ...
    const boss = e.kind === this.EK.Boss;
    const sheet = rogue ? 'rogue' : 'knight';
    const frame = rogue ? this.#rogueFrame(e) : this.#knightFrame(e);
    ...
    const top = (rogue ? 108 : 128) * e.scale;

    // Heavy attacks get an explicit "!" so they can be learned.
    if (e.state === FS.Attack && e.attackPhase === 0 && (e.attack === this.AK.KnightOverhead || e.attack === this.AK.KnightLunge)) {
  ...
  #drawVision(ctx, snap) {
    ...
    for (const e of snap.enemies) {
      ...
      const h = (e.kind === this.EK.Rogue ? 82 : e.kind === this.EK.Boss ? 135 : 100);
      const hw = e.kind === this.EK.Rogue ? 17 : e.kind === this.EK.Boss ? 34 : 24;
